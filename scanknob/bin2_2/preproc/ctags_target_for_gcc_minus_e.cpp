# 1 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox\\scanknob\\knobby2_2\\knobby2_2.ino"
# 1 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox\\scanknob\\knobby2_2\\knobby2_2.ino"

// Knobby2 - dlr (4/27/17)

// Uses Nextion display!
# 6 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox\\scanknob\\knobby2_2\\knobby2_2.ino" 2
# 7 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox\\scanknob\\knobby2_2\\knobby2_2.ino" 2

# 9 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox\\scanknob\\knobby2_2\\knobby2_2.ino" 2
...
volatile short int xdelta[4096], ydelta[4096], zdelta[4096], mem[4096]; // in micrometers...
    ...
    if (IRQsteps < delta_len) { // number of steps taken less than the length of the table
      ...
      if (IRQcount >= 2 * fjump[IRQsteps]) { // because it increments on both falling and raising edges
        ...
        if (zdelta[IRQsteps] != 0)
          z.write(z.read() + (int) ((float) zdelta[IRQsteps] / motor_gain[0] / mstep[vel][0] )); // simulate z step.... 12.8 is 1/gain_motor[0]

        if (ydelta[IRQsteps] != 0)
          y.write(y.read() + (int) ((float) ydelta[IRQsteps] / motor_gain[1] / mstep[vel][1] )); // same for y

        if (xdelta[IRQsteps] != 0)
          x.write(x.read() + (int) ((float) xdelta[IRQsteps] / motor_gain[2] / mstep[vel][2] )); // and x...
        ...
        switch (id) {
          ...
          case 11: //lock
            ...
            if (lock == 0) { // unlocking....
              ...
              p[3] = n[3] = a.read(); // ignore any knob movements during lock!
            }
            break;
...
//              }
  ...
  if (lock == 0 && page == 1) {
    ...
    n[0] = z.read(); n[1] = y.read(); n[2 /* motor axes*/] = x.read(); n[3] = a.read(); //read new position
