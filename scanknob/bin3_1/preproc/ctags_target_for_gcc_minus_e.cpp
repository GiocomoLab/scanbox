# 1 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox_Tower_v1\\scanknob\\knobby3_1\\knobby3_1.ino"
# 1 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox_Tower_v1\\scanknob\\knobby3_1\\knobby3_1.ino"
...
# 9 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox_Tower_v1\\scanknob\\knobby3_1\\knobby3_1.ino" 2
# 10 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox_Tower_v1\\scanknob\\knobby3_1\\knobby3_1.ino" 2

# 12 "C:\\Users\\dario\\Documents\\MATLAB\\Scanbox_Tower_v1\\scanknob\\knobby3_1\\knobby3_1.ino" 2
...
volatile boolean irq_state = 0x0; // attached/detached state
...
volatile short int xdelta[4096], ydelta[4096], zdelta[4096], mem[4096]; // in micrometers... or motor steps! (change in 2_3)
volatile byte xdelta_mode[4096], ydelta_mode[4096], zdelta_mode[4096];
volatile short int fjump[4096];
...
void format(char *fmt, ...) {
  ...
  vsnprintf((char *) buf, 32, fmt, args);
...
void setup() {
  ...
  pinMode(46, 0x1);
  digitalWrite(46, 0x0);
  attachInterrupt(((28) < (66u) ? (28) : -1), IRQcounter, 2);
...
void IRQcounter() {
  ...
  Serial1.print(String("arm.txt=\"Step ") + String(IRQsteps, 10) + String("\"\xff\xff\xff"));
    ...
    if (IRQsteps < delta_len) { // number of steps taken less than the length of the table
      ...
      if (IRQcount >= 2 * fjump[IRQsteps]) { // because it increments on both falling and raising edges
        ...
        if (zdelta[IRQsteps] != 0)
          z.write(z.read() + (int) (((float) zdelta[IRQsteps] / motor_gain[0] / mstep[vel][0])>=0?(long)(((float) zdelta[IRQsteps] / motor_gain[0] / mstep[vel][0])+0.5):(long)(((float) zdelta[IRQsteps] / motor_gain[0] / mstep[vel][0])-0.5))); // simulate z step.... 12.8 is 1/gain_motor[0]

        if (ydelta[IRQsteps] != 0)
          y.write(y.read() + (int) (((float) ydelta[IRQsteps] / motor_gain[1] / mstep[vel][1])>=0?(long)(((float) ydelta[IRQsteps] / motor_gain[1] / mstep[vel][1])+0.5):(long)(((float) ydelta[IRQsteps] / motor_gain[1] / mstep[vel][1])-0.5))); // same for y

        if (xdelta[IRQsteps] != 0)
          x.write(x.read() + (int) (((float) xdelta[IRQsteps] / motor_gain[2] / mstep[vel][2])>=0?(long)(((float) xdelta[IRQsteps] / motor_gain[2] / mstep[vel][2])+0.5):(long)(((float) xdelta[IRQsteps] / motor_gain[2] / mstep[vel][2])-0.5))); // and x...
  ...
  if (oldpage != page) {
    ...
    if (page == 1) { // we are entering page #1
      ...
      if (visitvel) { // if the velocity buttons were used...
        ...
        n[0] = n[1] = n[2 /* motor axes*/] = n[3] = 0;
        p[0] = p[1] = p[2 /* motor axes*/] = p[3] = 0;
        dpos[0] = dpos[1] = dpos[2 /* motor axes*/] = dpos[3] = 0;
        cmd[0] = 12;
      ...
      case 1: // main page
        ...
        switch (id) {
          ...
          case 34: // virtual alpha
            ...
            n[0] = p[0] = 0; // reset z counter and new/past positions when switching between modes
            n[3] = p[3] = 0;
            z.write(0);
          ...
          case 10: //lock
            ...
            if (lock == 0) { // unlocking....
              p[0] = n[0] = z.read();
              p[1] = n[1] = y.read();
              p[2 /* motor axes*/] = n[2 /* motor axes*/] = x.read();
              p[3] = n[3] = z.read(); // ignore any knob movements during lock!
            }
            break;
...
//        }
      ...
      case 4: // zero menu page
        ...
        switch (id) {
          ...
          case 1:
            ...
            n[0] = n[1] = n[2 /* motor axes*/] = 0;
            p[0] = p[1] = p[2 /* motor axes*/] = 0;
            dpos[0] = dpos[1] = dpos[2 /* motor axes*/] = 0;
      ...
      case 6: // store menu page
        ...
        switch (id) {
          ...
          case 4: // recall a
            mpos[0][2 /* motor axes*/] = dpos[2 /* motor axes*/];
            mpos[0][1] = dpos[1];
            mpos[0][0] = dpos[0];
            mpos[0][3] = dpos[3];
            oldpage = 6;
          ...
          case 1: // recall b
            mpos[1][2 /* motor axes*/] = dpos[2 /* motor axes*/];
            mpos[1][1] = dpos[1];
            mpos[1][0] = dpos[0];
            mpos[1][3] = dpos[3];
            oldpage = 6;
          ...
          case 2: // recall c
            mpos[2][2 /* motor axes*/] = dpos[2 /* motor axes*/];
            mpos[2][1] = dpos[1];
            mpos[2][0] = dpos[0];
            mpos[2][3] = dpos[3];
            oldpage = 6;
      ...
      case 5: // recall menu page
        switch (id) {
          ...
          case 4:
            dpos[2 /* motor axes*/] = mpos[0][2 /* motor axes*/];
            dpos[1] = mpos[0][1];
            dpos[0] = mpos[0][0];
            dpos[3] = mpos[0][3];
            oldpage = 5;
          ...
          case 1:
            dpos[2 /* motor axes*/] = mpos[1][2 /* motor axes*/];
            dpos[1] = mpos[1][1];
            dpos[0] = mpos[1][0];
            dpos[3] = mpos[1][3];
            oldpage = 5;
          ...
          case 2:
            dpos[2 /* motor axes*/] = mpos[2][2 /* motor axes*/];
            dpos[1] = mpos[2][1];
            dpos[0] = mpos[2][0];
            dpos[3] = mpos[2][3];
            oldpage = 5;
  ...
  if (lock == 0 && page == 1) {
    ...
    if (valpha == 0) {
      n[0] = z.read(); n[1] = y.read(); n[2 /* motor axes*/] = x.read(); //read new position
    } else {
      n[3] = z.read(); n[1] = y.read(); n[2 /* motor axes*/] = x.read(); //read new position
    }
